...
namespace aspnet.Services.shares
{
    ...
    public class mas
    {
        ...
        [OperationContract]
        public string get_salesman(string connectString)
        {
            List<vchart.gr_var> data = new List<vchart.gr_var>();
            ...
            using (masDataContext dc = new masDataContext(connectString))
            {
                ...
                {
                    dc.Connection.Open();
                    var q = from c in dc.sales
                            orderby c.zone, c.salesman ascending
                            group c by new { c.salesman } into g
                            select new
                            {
                                 g.Key.salesman
                            };
                    ...
                    if (q.Count() > 0)
                    {
                        foreach (var d in q)
                        {
                            var qs = from c in dc.sales
                                     where  c.salesman == d.salesman
                                     orderby c.month ascending
                            ...
                            for (int i = 1; i <= 12; i++)
                            {
                                ...
                                var qf = from a in qs
                                         where a.month == i
                                         select a;
                                if (qf.Count() > 0) amt = qf.First().amount.ToString();
                                value = value + amt + ",";
                            }

                            data.Add(new vchart.gr_var()
                            {
                                salesman = d.salesman,
                                addval = value.Remove(value.LastIndexOf(','))
                            });
                        }
                    }
    ...
    public static class vchart
    {
        public class gr_var
        {
            public string salesman { get; set; } //series
